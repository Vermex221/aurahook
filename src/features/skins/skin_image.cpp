...
namespace features::skins::skin_img {
    namespace {
        ...
        std::filesystem::path cs2_cache_dir() {
        ...
        std::filesystem::path aura_cache_dir() {
            char* user_profile = nullptr;
            size_t len = 0;
            std::filesystem::path folder;
            if (_dupenv_s(&user_profile, &len, "USERPROFILE") == 0 && user_profile && len > 0) {
                folder = user_profile; free(user_profile);
                folder /= "Documents"; folder /= "aurahook"; folder /= "skinimgs";
            } else {
                folder = "configs"; folder /= "skinimgs";
            }
            std::error_code ec;
            std::filesystem::create_directories(folder, ec);
            return folder;
        ...
        bool load_png_from_disk(const std::string& key, std::vector<std::uint8_t>& out) {
            ...
            paths.push_back(cs2_cache_dir() / "imgs" / (key_to_file_id(key) + ".png"));
            paths.push_back(aura_cache_dir() / (key + ".png"));
            paths.push_back(std::filesystem::path("configs/skinimgs") / (key + ".png"));
            ...
            for (const auto& path : paths) {
                ...
                if (in.good() && !out.empty()) return true;
            }
            return false;
        }
        ...
        bool ensure_bymykel_index() {
            const auto cache_path = cs2_cache_dir() / "bymykel_images.json";
            const auto skins_cache = cs2_cache_dir() / "bymykel_skins.json";
        ...
        std::string find_generated_key(const char* stem, const char* kit) {
            ...
            if (found.empty()) {
                if (!k.empty()) found = "econ/default_generated/" + s + "_" + k + "_light";
                else found = "econ/weapons/base_weapons/" + s;
            }
        ...
        ID3D11ShaderResourceView* resolve_srv(const std::string& key_in) {
            ...
            if (entry.state == tex_state::empty) {
                ...
                if (entry.url.empty()) entry.url = key;
    ...
    ImTextureID get_weapon(const char* weapon_name) {
        if (!weapon_name || !*weapon_name) return (ImTextureID)0;
        return get_model_any(weapon_name);
    }
