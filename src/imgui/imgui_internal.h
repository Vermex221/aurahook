...
#define IM_DEBUG_BREAK()    __asm__ volatile(".inst 0xd4200000")    
#else
...
IMGUI_API const char*   ImTextFindPreviousUtf8Codepoint(const char* in_text_start, const char* in_p);                           
IMGUI_API const char*   ImTextFindValidUtf8CodepointEnd(const char* in_text_start, const char* in_text_end, const char* in_p);  
...
IMGUI_API const char*   ImTextCalcWordWrapNextLineStart(const char* text, const char* text_end, ImDrawTextFlags flags = 0); 
...
inline float  ImCeilFast(float f)                                       { int i = (int)f; return (float)(i + (f > (float)i)); } 
...
IMGUI_API ImVec2     ImBezierCubicClosestPointCasteljau(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, const ImVec2& p, float max_error);
...
struct ImBitArray
{
    ...
    void            SetBitRange(int n, int n2)  { n += OFFSET; n2 += OFFSET; IM_ASSERT(n >= 0 && n < BITCOUNT && n2 > n && n2 <= BITCOUNT); ImBitArraySetBitRange(Data, n, n2); } 
...
struct ImGuiPackedDate
{
    ...
    ImGuiPackedDate(int yyyymmdd)               { Year = (ImU16)((yyyymmdd / 10000) - 2000); Month = (ImU16)((yyyymmdd / 100) % 100); Day = (ImU16)(yyyymmdd % 100); } 
    bool                IsValid()               { return (Year && Month && Day); }
    int                 Unpack() const          { return (Year && Month && Day) ? ((Year + 2000) * 10000) + (Month * 100) + Day : 0; }      
    void                SubtractMonths(int m)   { while (m > 0) { Year -= Month == 1; Month = (Month == 1) ? 12 : Month - 1; m--; } }       
};
...
struct IMGUI_API ImDrawListSharedData
{
    ...
    float           FontSize;                   
    float           FontScale;                  
    float           CurveTessellationMaxError;  
...
struct ImGuiNextWindowData
{
    ...
    ImVec2                      ScrollVal;
    ImGuiWindowFlags            WindowFlagsSet;         
    ImGuiWindowFlags            WindowFlagsClear;
    ImGuiChildFlags             ChildFlagsSet;          
...
struct ImGuiNextItemData
{
    ...
    inline void ClearFlags()    { HasFlags = ImGuiNextItemDataFlags_None; ItemFlagsSet = ImGuiItemFlags_None; } 
};
...
#define ImGuiKey_NavGamepadContextMenu  ImGuiKey_GamepadFaceUp      
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    int                 BgFgDrawListsLastFrameActive[2]; 
    ...
    ImGuiViewportP()    { BgFgDrawLists[0] = BgFgDrawLists[1] = NULL; BgFgDrawListsLastFrameActive[0] = BgFgDrawListsLastFrameActive[1] = -1; BgFgDrawListsLastTimeActive[0] = BgFgDrawListsLastTimeActive[1] = -1.0; }
...
{
    ImGuiID         TypeHashFilter = 0;                             
    int             DiscardOlderThanMonths = 0;                     
    bool            DiscardWhenMissingDate = false;                 
    bool            DiscardAll = false;                             
    bool            SetCurrentSessionDateToAll = false;             
    bool            SetCurrentSessionDateWhenMissingDate = false;   
    int             _DiscardOlderThanDate = 0;                      
};
...
struct ImGuiSettingsHandler
{
    ...
    void        (*CleanupFn) (ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiSettingsCleanupArgs* args);
    void*       UserData;
...
namespace ImGui
{
    ...
    IMGUI_API void          SetPixelDensity(float pixel_density); 
    ...
    IMGUI_API void                  CleanupIniSettings(ImGuiSettingsCleanupArgs* args); 
    ...
    IMGUI_API void          RenderNavCursor(const ImRect& bb, ImGuiID id, ImGuiNavRenderCursorFlags flags = ImGuiNavRenderCursorFlags_None, float rounding = -1.0f); 
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    ...
    IMGUI_API void          TextAligned(float align_x, float width, const char* fmt, ...) IM_FMTARGS(3); 
...
#define ImFontAtlasRectId_IndexMask_        (0x0007FFFF)    
