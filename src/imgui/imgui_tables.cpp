...
void    ImGui::EndTable()
{
    ...
    const float inner_content_max_y = ImCeil(table->RowPosY2); 
...
void ImGui::TableReconcileColumns(ImGuiTable* table)
{
    ...
    for (ImGuiTableReconcileColumnData& reconcile_data : reconcile_requests)
    {
        table->Columns[reconcile_data.ColumnNewIdx] = reconcile_data.ColumnOldData; 
        ...
        IMGUI_DEBUG_LOG_TABLE("[table] - old %d -> new %d \"%s\"\n", reconcile_data.ColumnOldIdx, reconcile_data.ColumnNewIdx, TableGetColumnName(table, reconcile_data.ColumnNewIdx)); 
    }
    TableFixDisplayOrder(table);
    table->IsSettingsDirty = true; 
    table->IsReconcileMode = false;
    reconcile_requests.resize(0); 
}
...
static bool MenuItemForColumnReorder(const char* label, bool selected, bool enabled)
{
    ...
    float min_w = offsets->DeclColumns(0.0f, label_size.x, 0.0f, checkmark_w); 
    ...
    const bool has_been_moved = (g.ActiveId == id) && g.ActiveIdHasBeenEditedBefore; 

    BeginDisabled(!enabled); 
    bool ret = Selectable(label, false, selectable_flags, ImVec2(min_w, label_size.y)) && !has_been_moved; 
...
void ImGui::TableDrawDefaultContextMenu(ImGuiTable* table, ImGuiTableFlags flags_for_section_to_display)
{
    ...
    if (flags_for_section_to_display & ImGuiTableFlags_Hideable)
    {
        ...
        const bool is_reordering = (g.ActiveId != 0 && g.ActiveIdWindow == g.CurrentWindow && table->ReorderColumn != -1 && g.ActiveIdHasBeenEditedBefore); 
        ...
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            if (IsItemActive() && IsMouseDragging(0) && g.ActiveIdSource == ImGuiInputSource_Mouse && (table->Flags & ImGuiTableFlags_Reorderable))
            {
                ...
                if (!IsItemHovered())
                {
                    ...
                    int dst_order = column->DisplayOrder + (int)ImCeil(reorder_amount) * reorder_dir; 
...
void ImGui::TableLoadSettingsForColumns(ImGuiTable* table)
{
    ...
    if (matches == settings->ColumnsCount)
        return;
    const int settings_start_n = matches; 
...
void ImGui::TableLoadSettingsForColumn(ImGuiTableColumn* column, ImGuiTableColumnSettings* column_settings, ImGuiTableFlags load_flags)
{
    ...
    if (load_flags & ImGuiTableFlags_Reorderable)
        ...
        column->DisplayOrder = column_settings->Index; 
...
{
    ImGuiContext& g = *GImGui;
    g.TempBuffer.reserve((int)(sizeof(ImGuiTableFixDisplayOrderColumnData) * table->ColumnsCount)); 
...
static void TableSettingsHandler_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line)
{
    ...
    if (sscanf(line, "Column %d%n", &column_n, &r) == 1)
    {
        ...
        if (sscanf(line, "UserID=0x%08X%n", (ImU32*)&n, &r)==1) { line = ImStrSkipBlank(line + r); } 
