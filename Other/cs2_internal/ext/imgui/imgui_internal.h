...
#include <type_traits>  // std::is_arithmetic_v (ImLerp scalar constraint)
...
// Restrict scalar ImLerp so IMGUI_DEFINE_MATH_OPERATORS + ImVec4 don't instantiate (float)ImVec4
template<typename T> requires (std::is_arithmetic_v<T>)
T ImLerp(T a, T b, float t)                                            { return (T)((float)a + (float)(b - a) * t); }
