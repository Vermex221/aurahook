...
struct ImColor
{
    ...
    // noobchair: multiply alpha (used by ESP / overlays)
    inline ImColor AlphaOverride(float alpha_mul) const             { return ImColor(Value.x, Value.y, Value.z, Value.w * alpha_mul); }
...
enum ImDrawFlags_
{
    ...
    ImDrawFlags_ShadowCutOutShapeBackground = 1 << 10, // noobchair compat: shadows-branch flag (stubbed)
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddShadowConvexPoly(const ImVec2* points, int points_count, ImU32 col, float shadow_thickness = 1.0f, const ImVec2& shadow_offset = ImVec2(0,0), ImDrawFlags flags = 0);
    IMGUI_API void  AddShadowRect(const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float shadow_thickness = 1.0f, const ImVec2& shadow_offset = ImVec2(0,0), ImDrawFlags flags = 0, float rounding = 0.0f);
