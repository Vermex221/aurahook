...
namespace xdraw {
	...
	namespace {
		inline ImU32 to_im(color c) { return IM_COL32(c.r, c.g, c.b, c.a); }
	}
	...
	namespace detail {
		...
		static void ensure_blur_textures( )
		{
			const int w = g.surface_w;
			const int h = g.surface_h;
			if ( w <= 0 || h <= 0 )
				return;

			if ( g.blur_scene_tex && w == g.blur_cached_w && h == g.blur_cached_h )
				return;

			create_blur_textures( w, h );
		}
	...
	bool font::rasterize( char32_t cp )
	{
		...
		if ( this->hinted_aa )
		{
			load_flags = FT_LOAD_RENDER | FT_LOAD_TARGET_NORMAL | FT_LOAD_FORCE_AUTOHINT;
		}
		else if ( this->pixel_perfect )
		{
			load_flags = FT_LOAD_RENDER | FT_LOAD_TARGET_MONO | FT_LOAD_MONOCHROME | FT_LOAD_NO_HINTING | FT_LOAD_NO_AUTOHINT;
		}
		else
		{
			load_flags = FT_LOAD_RENDER | FT_LOAD_TARGET_NORMAL | FT_LOAD_FORCE_AUTOHINT;
		...
		for ( int y = 0; y < gh; ++y )
		{
			for ( int x = 0; x < gw; ++x )
			{
				...
				if ( mono )
				{
					const auto byte = bmp.buffer[ y * bmp.pitch + ( x >> 3 ) ];
	...
	void draw_list::push_clip( float x, float y, float w, float h )
	{
		if ( this->imgui )
		{
			this->imgui->PushClipRect( ImVec2( x, y ), ImVec2( x + w, y + h ), true );
			this->clip_stack.push_back( {} );
			return;
		}
	...
	void draw_list::pop_clip( )
	{
		if ( this->imgui )
		{
			this->imgui->PopClipRect( );
			if ( !this->clip_stack.empty() ) this->clip_stack.pop_back();
			return;
		}
	...
	void draw_list::rect_filled( float x, float y, float w, float h, color col )
	{
		...
		if ( this->imgui )
		{
			this->imgui->AddRectFilled( ImVec2( x, y ), ImVec2( x + w, y + h ), to_im( col ) );
			return;
		}
	...
	void draw_list::rect_filled_gradient( float x, float y, float w, float h, color tl, color tr, color br, color bl )
	{
		...
		if ( this->imgui )
		{
			this->imgui->AddRectFilledMultiColor( ImVec2( x, y ), ImVec2( x + w, y + h ), to_im( tl ), to_im( tr ), to_im( br ), to_im( bl ) );
			return;
		}
	...
	void draw_list::rect_filled_blurred( float x, float y, float w, float h, color tint )
	{
		if ( this->imgui )
		{
			this->rect_filled( x, y, w, h, color{ 20, 20, 20, static_cast<std::uint8_t>(180 * tint.a / 255) } );
			return;
		}
		...
		if ( !blur_srv )
		{
			detail::ensure_blur_textures( );
			blur_srv = detail::g.blur_chain[ 0 ].srv.Get( );
			if ( !blur_srv )
				return;
		}
	...
	void draw_list::circle_filled( float cx, float cy, float radius, color col, int segments, bool aa )
	{
		if ( this->imgui )
		{
			this->imgui->AddCircleFilled( ImVec2( cx, cy ), radius, to_im( col ), segments > 0 ? segments : 0 );
			return;
		}
	...
	void draw_list::triangle_filled( float x0, float y0, float x1, float y1, float x2, float y2, color col, bool aa )
	{
		if ( this->imgui )
		{
			this->imgui->AddTriangleFilled( ImVec2( x0, y0 ), ImVec2( x1, y1 ), ImVec2( x2, y2 ), to_im( col ) );
			return;
		}
	...
	void draw_list::triangle_filled_multi( float x0, float y0, color c0, float x1, float y1, color c1, float x2, float y2, color c2 )
	{
		if ( this->imgui )
		{
			this->imgui->PrimReserve( 3, 3 );
			const auto uv = this->imgui->_Data->TexUvWhitePixel;
			this->imgui->PrimVtx( ImVec2( x0, y0 ), uv, to_im( c0 ) );
			this->imgui->PrimVtx( ImVec2( x1, y1 ), uv, to_im( c1 ) );
			this->imgui->PrimVtx( ImVec2( x2, y2 ), uv, to_im( c2 ) );
			return;
		}
		...
		this->ensure_cmd( nullptr );
		const auto a = this->emit_vtx( x0, y0, 0.0f, 0.0f, c0 );
		const auto b = this->emit_vtx( x1, y1, 0.0f, 0.0f, c1 );
		const auto c = this->emit_vtx( x2, y2, 0.0f, 0.0f, c2 );
		this->emit_idx( a, b, c );
	}
	...
	void draw_list::rect( float x, float y, float w, float h, color col, float thickness, bool aa )
	{
		if ( this->imgui )
		{
			this->imgui->AddRect( ImVec2( x, y ), ImVec2( x + w, y + h ), to_im( col ), 0.0f, 0, thickness );
			return;
		}
	...
	void draw_list::circle( float cx, float cy, float radius, color col, float thickness, int segments, bool aa )
	{
		if ( this->imgui )
		{
			this->imgui->AddCircle( ImVec2( cx, cy ), radius, to_im( col ), segments > 0 ? segments : 0, thickness );
			return;
		}
	...
	void draw_list::line( float x0, float y0, float x1, float y1, color col, float thickness, bool aa )
	{
		if ( this->imgui )
		{
			this->imgui->AddLine( ImVec2( x0, y0 ), ImVec2( x1, y1 ), to_im( col ), thickness );
			return;
		}
	...
	{
		if ( this->imgui && points.size() >= 4 )
		{
			const auto n = points.size() / 2;
			for ( size_t i = 0; i + 1 < n; ++i )
			{
				this->imgui->AddLine( ImVec2( points[i*2], points[i*2+1] ), ImVec2( points[(i+1)*2], points[(i+1)*2+1] ), to_im( col ), thickness );
			}
			if ( closed )
			{
				this->imgui->AddLine( ImVec2( points[(n-1)*2], points[(n-1)*2+1] ), ImVec2( points[0], points[1] ), to_im( col ), thickness );
			}
			return;
		}
	...
	void draw_list::text( float x, float y, std::string_view str, color col, font* f )
	{
		if ( this->imgui )
		{
			this->imgui->AddText( ImVec2( x, y ), to_im( col ), str.data(), str.data() + str.size() );
			return;
		}
	...
	void draw_list::text( float x, float y, std::string_view str, color col, text_style style, color shadow_col, font* f )
	{
		...
		case text_style::outlined:
		{
			...
			const auto outline_col = shadow_col.a > 0 ? shadow_col : color{ 0, 0, 0, col.a };
			for ( const auto& [ox, oy] : offsets )
			{
				this->text( x + ox, y + oy, str, outline_col, f );
			}
	...
	void draw_list::image( float x, float y, float w, float h, ID3D11ShaderResourceView* tex, color tint )
	{
		if ( this->imgui && tex )
		{
			this->imgui->AddImage( (ImTextureID)tex, ImVec2( x, y ), ImVec2( x + w, y + h ), ImVec2( 0, 0 ), ImVec2( 1, 1 ), to_im( tint ) );
			return;
		}
	...
	void set_display_metrics( int width, int height )
	{
		auto& d = detail::g;
		d.surface_w = width > 0 ? width : 0;
		d.surface_h = height > 0 ? height : 0;
	}
	...
	void end_frame( )
	{
		...
		for ( auto i = 0; i < 3; ++i )
		{
			if ( !d.lists[ i ].vertices.empty( ) || !d.glow_lists[ i ].vertices.empty( ) )
			{
				has_work = true;
				break;
			}
		...
		const float w = d.surface_w > 0 ? static_cast< float >( d.surface_w ) : vp.Width;
		const float h = d.surface_h > 0 ? static_cast< float >( d.surface_h ) : vp.Height;
		if ( w <= 1.0f || h <= 1.0f )
			return;

		D3D11_VIEWPORT draw_vp{};
		draw_vp.Width = w;
		draw_vp.Height = h;
		draw_vp.MaxDepth = 1.0f;
		d.context->RSSetViewports( 1, &draw_vp );

		D3D11_RECT full_scissor{};
		full_scissor.right = static_cast< LONG >( std::ceil( w ) );
		full_scissor.bottom = static_cast< LONG >( std::ceil( h ) );

		const auto tex_w = static_cast< int >( w );
		const auto tex_h = static_cast< int >( h );
		if ( d.blur_scene_tex && tex_w > 0 && tex_h > 0 && ( tex_w != d.blur_cached_w || tex_h != d.blur_cached_h ) )
			detail::create_blur_textures( tex_w, tex_h );
		...
		auto render_layer = [ & ]( draw_list& dl )
			{
				...
				for ( auto i = 0ull; i < dl.commands.size( ); ++i )
				{
					...
					{
						scissor = cmd.scissor;
						scissor.left = ( std::max )( full_scissor.left, scissor.left );
						scissor.top = ( std::max )( full_scissor.top, scissor.top );
						scissor.right = ( std::min )( full_scissor.right, scissor.right );
						scissor.bottom = ( std::min )( full_scissor.bottom, scissor.bottom );
		...
		ID3D11ShaderResourceView* null_srv{};
		d.context->PSSetShaderResources( 0, 1, &null_srv );
	...
	font* load_font( std::span<const std::byte> data, float size_px, int atlas_w, int atlas_h, bool pixel_perfect, bool hinted_aa )
	{
		FT_Library ft{};
		...
		f->atlas_w = atlas_w;
		f->atlas_h = atlas_h;
		f->pixel_perfect = pixel_perfect;
		f->hinted_aa = hinted_aa;
		...
		if ( hinted_aa )
		{
			for ( char32_t cp = 0x00A0; cp <= 0x00FF; ++cp )
			{
				f->rasterize( cp );
			}
			for ( char32_t cp = 0x0400; cp <= 0x04FF; ++cp )
			{
				f->rasterize( cp );
			}
		}
	...
	{
		auto& d = detail::g;
		if ( d.surface_w > 0 && d.surface_h > 0 )
			return { d.surface_w, d.surface_h };
