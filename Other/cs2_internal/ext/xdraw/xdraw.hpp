...
namespace xdraw {
	...
	struct draw_list
	{
		...
		void triangle_filled_multi( float x0, float y0, color c0, float x1, float y1, color c1, float x2, float y2, color c2 );
	...
	[[nodiscard]] font* load_font( std::span<const std::byte> data, float size_px, int atlas_w = 1024, int atlas_h = 1024, bool pixel_perfect = false, bool hinted_aa = false );
...
namespace tokens {
	...
	inline xdraw::color col_accent{ 168, 85, 247, 255 };
	inline xdraw::color col_dark{ 17, 17, 17, 255 };
	inline xdraw::color col_text{ 221, 229, 255, 235 };
	inline xdraw::color col_text_dim{ 168, 85, 247, 82 };
