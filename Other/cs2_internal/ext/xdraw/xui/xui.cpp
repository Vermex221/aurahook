...
namespace xui {
	...
	bool feed_wndproc( input_state& s, UINT msg, WPARAM wp, LPARAM lp )
	{
		auto apply_mouse = [ & ]( )
		{
			...
			if ( client_w < 1.0f || client_h < 1.0f || display.first <= 0 || display.second <= 0 )
				return;

			s.mouse_x *= static_cast< float >( display.first ) / client_w;
			s.mouse_y *= static_cast< float >( display.second ) / client_h;
		};
