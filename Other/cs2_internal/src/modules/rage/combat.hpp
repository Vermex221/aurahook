...
namespace features::combat {
	...
	constexpr auto k_pen_crosshair_brush_mask{ 0x101001ull };
	...
	public:
		...
		public:
			...
			struct extrapolation_data
			{
				...
				std::uintptr_t trace_mask{};
			};

			void run( );
			void clear( );

			[[nodiscard]] std::optional<record> get_oldest_valid( std::uintptr_t pawn ) const;
			[[nodiscard]] std::optional<record> get_oldest_was_valid( std::uintptr_t pawn ) const;
			[[nodiscard]] std::optional<visual_record> get_oldest_was_valid_visual( std::uintptr_t pawn ) const;
			[[nodiscard]] std::vector<record> get_valid_records( std::uintptr_t pawn ) const;
			[[nodiscard]] std::vector<record> get_valid_records_for_extrapolation( std::uintptr_t pawn ) const;
			[[nodiscard]] std::vector<record_tick_info> collect_valid_tick_infos( std::uintptr_t pawn ) const;
		...
		private:
			[[nodiscard]] std::vector<record> collect_valid_records( std::uintptr_t pawn, int max_ticks ) const;
			...
			std::unordered_set<std::uintptr_t> m_run_active{};
			std::vector<pending_record> m_run_pending{};
		...
		public:
			...
			[[nodiscard]] run_context prepare_target( std::uintptr_t target_pawn, const lagcomp::record* record ) const;
		...
		[[nodiscard]] const lagcomp::record* current_autowall_record( ) const { return this->m_current_autowall_record; }
		...
		[[nodiscard]] bool find_spread_correction( const math::vector3& aim_angle, int tick, math::vector3& out ) const;
	...
	private:
		...
		inline static thread_local const lagcomp::record* m_current_autowall_record{ nullptr };
	...
	private:
		...
		public:
			...
			[[nodiscard]] bool active( ) const { return this->m_antiaim_active; }
			...
			[[nodiscard]] bool mouse_override_capturing( ) const { return this->m_mouse_override_active; }
			void apply_mouse_override_view_freeze( ) const
			{
		...
		private:
			...
			void update_mouse_override( float view_yaw );
			void release_mouse_override_cursor( );
			void draw_mouse_override_indicator( xdraw::draw_list& draw_list ) const;
			...
			bool m_yaw_jitter_flip{};
			bool m_pitch_jitter_flip{};
			float m_spin{};
			bool m_mouse_override_active{};
			bool m_mouse_override_centered{};
			bool m_mouse_override_latched{};
			bool m_mouse_override_was_held{};
			bool m_mouse_override_cursor_owned{};
			std::uint8_t m_mouse_override_saved_relative{};
			math::vector3 m_mouse_override_frozen_view{};
			float m_mouse_override_yaw{};
			float m_mouse_override_x{};
			float m_mouse_override_y{};
			float m_mouse_override_radius{};
			unsigned long long m_mouse_override_last_press_ms{};
		};
	...
	public:
		...
		[[nodiscard]] autopeak& autopeak( ) { return this->m_autopeak; }
	...
	public:
		...
		static constexpr auto k_max_scan_records{ 8 };
	...
	private:
		...
		struct candidate
		{
			...
			int record_count{};
			std::array<shared::lagcomp::record*, k_max_lagcomp_records + 1> all_records{};
			int all_record_count{};
		};
		...
		[[nodiscard]] std::vector<shared::lagcomp::record> collect_candidate_records( std::uintptr_t pawn ) const;
		[[nodiscard]] int pick_candidate_record_indices( const std::vector<shared::lagcomp::record>& records, std::array<int, k_max_scan_records>& out_indices ) const;
		...
		[[nodiscard]] bool try_auto_scope( systems::input::usercmd* cmd ) const;
		...
		[[nodiscard]] const std::vector<scan_hit>& scan_player( const math::vector3& eye, float inaccuracy, const aim_context& ctx, candidate& cand, shared::lagcomp::record* record, const systems::local::snapshot& local ) const;
		[[nodiscard]] target select_best( const aim_context& aim_ctx, const std::vector<scan_hit>& hits, float eval_inaccuracy, const std::vector<candidate>& candidates, const systems::local::snapshot& local, const selection_options& options ) const;
		[[nodiscard]] bool verify_safe_point( const scan_hit& hit, const candidate& cand, const systems::local::snapshot& local, float threshold, float& out_min_damage ) const;
		...
		[[nodiscard]] const std::vector<math::vector3>& generate_multipoints( const systems::hitboxes::entry& hitbox, const math::vector3& center, const math::quaternion& bone_rot, float pointscale, const math::vector3& shoot_pos, float inaccuracy ) const;
		...
		void sample_penetration_crosshair( const systems::local::snapshot& local );
		...
		void draw_visualize_aimbot( );
		void update_visualize_target( const scan_hit& hit );
		void clear_visualize_target( );
		...
		pen_crosshair_result m_penetration_crosshair{};
		std::atomic<bool> m_pen_can_pen{ false };

		struct visualize_target
		{
			math::vector3 start{};
			math::vector3 end{};
			float radius{};
		...
		visualize_target m_visualize{};

		std::vector<shared::lagcomp::record> m_candidate_records{};

	public:
		void push_visualize( const scan_hit& hit );
	};
	...
	private:
		...
		[[nodiscard]] scan_point scan_player( std::uintptr_t pawn, const shared::lagcomp::record& record, const math::vector3& shoot_position, const math::vector3& view_angles, const settings::combat::legitbot::weapon_group& config, const systems::local::snapshot& local ) const;
		...
		void apply_standalone_backtrack( systems::input::usercmd* cmd, const math::vector3& shoot_position, const math::vector3& view_angles, const systems::local::snapshot& local );
		...
		std::uintptr_t m_last_local_pawn{};
