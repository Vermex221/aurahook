...
namespace features::movement {
	...
	void test_strafer::quantized_path( systems::input::usercmd* cmd )
	{
		...
		if ( !features::combat::g_misc.antiaim( ).active( ) )
		{
			base->mutable_viewangles( )->set_y( camera_yaw );
		}

		const auto command_yaw = base->viewangles( )->y( );
		auto aa_yaw_delta = command_yaw - camera_yaw;
	math::helpers::normalize_angle( aa_yaw_delta );
		...
		auto target_yaw = camera_yaw + base_yaw_offset;
		...
		for ( auto i = 1; i <= k_max_subticks; ++i )
		{
			...
			auto target_camera_yaw = wishdir_yaw - base_yaw_offset;
		math::helpers::normalize_angle( target_camera_yaw );

			auto target_cmd_yaw = target_camera_yaw + aa_yaw_delta;
			math::helpers::normalize_angle( target_cmd_yaw );

			auto yaw_delta = target_cmd_yaw - acc_yaw;
			math::helpers::normalize_angle( yaw_delta );
