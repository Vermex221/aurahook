...
namespace features::movement {
	...
	void airstrafe::on_create_move(systems::input::usercmd* cmd)
	{
		...
		const auto effective_maxspeed = reinterpret_cast<CPlayer_MovementServices*>(movement_services)->m_flMaxspeed();
	...
	void airstrafe::rotate_to_stop(proto::base_usercmd_pb* base, const math::vector3& velocity) const
	{
...

		{
			...
			const auto max_speed = (ctx.valid && ctx.weapon_vdata) ? reinterpret_cast<CCSWeaponBaseVData*>(ctx.weapon_vdata)->m_flMaxSpeed().value() : 250.0f;
