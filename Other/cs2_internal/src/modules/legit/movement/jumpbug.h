...
namespace features::movement {
	...
	void jumpbug::on_create_move( systems::input::usercmd* cmd )
	{
		...
		const auto duck_amount = reinterpret_cast<CCSPlayer_MovementServices*>( movement_services )->m_flDuckAmount();
		const auto duck_speed = reinterpret_cast<CCSPlayer_MovementServices*>( movement_services )->m_flDuckSpeed();
		...
		auto maxs = reinterpret_cast<CCollisionProperty*>( collision )->m_vecMaxs();
		...
		const auto gravity_scale = reinterpret_cast<C_BaseEntity*>( local.pawn )->m_flGravityScale();
	...
	{
		const auto movement_services = reinterpret_cast<C_BasePlayerPawn*>( local_pawn )->m_pMovementServices();
		if ( !movement_services )
		{
			return 0.0f;
		}

		const auto stamina = reinterpret_cast<CCSPlayer_MovementServices*>( movement_services )->m_flStamina();
		...
		const auto modern_jump_ptr = reinterpret_cast<std::uintptr_t>( &reinterpret_cast<CCSPlayer_MovementServices*>( movement_services )->m_ModernJump() );
		const auto landing_vel_z = reinterpret_cast<CCSPlayerModernJump*>( modern_jump_ptr )->m_flLastLandedVelocityZ();
		const auto landed_tick = reinterpret_cast<CCSPlayerModernJump*>( modern_jump_ptr )->m_nLastLandedTick();
