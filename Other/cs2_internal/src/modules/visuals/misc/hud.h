...
namespace features::misc {
	...
	void hud::do_scope( xdraw::draw_list& draw_list, float cx, float cy, float screen_h, std::uintptr_t local_pawn )
	{
		...
		{
			this->m_scope_anim = std::lerp( this->m_scope_anim, 0.0f, std::min( xdraw::delta_time( ) * 8.0f, 1.0f ) );
			return;
		}

		const auto is_scoped = reinterpret_cast<C_CSPlayerPawn*>(local_pawn)->m_bIsScoped();
		const auto anim_speed = std::max( cfg.anim_speed.value, 8.0f );

		this->m_scope_anim = std::lerp( this->m_scope_anim, is_scoped ? 1.0f : 0.0f, std::min( xdraw::delta_time( ) * anim_speed, 1.0f ) );
		if ( this->m_scope_anim < 0.01f )
		{
		...
		if ( cfg.type.value == settings::misc::hud::scope::scope_type::full )
		{
			const auto [screen_w, _] = xdraw::viewport_size( );
			const auto alpha = static_cast< std::uint8_t >( 255.0f * this->m_scope_anim );
			const auto col = xdraw::color{ 0, 0, 0, alpha };
			draw_list.line( 0.0f, cy, static_cast< float >( screen_w ), cy, col, 1.0f );
			draw_list.line( cx, 0.0f, cx, screen_h, col, 1.0f );
			return;
		}
		...
		if ( should_recalc )
		{
			...
			const auto weapon_services = reinterpret_cast<C_BasePlayerPawn*>(local_pawn)->m_pWeaponServices();
			if ( weapon_services )
			{
				...
				if ( weapon )
				{
					...
					if ( weapon_vdata )
					{
						const auto vel = reinterpret_cast<C_BaseEntity*>(local_pawn)->m_vecAbsVelocity();
						const auto fire_mode = reinterpret_cast<C_CSWeaponBase*>(weapon)->m_weaponMode();
						const auto accuracy_penalty = reinterpret_cast<C_CSWeaponBase*>(weapon)->m_fAccuracyPenalty();
						const auto turning_inaccuracy = reinterpret_cast<C_CSWeaponBase*>( weapon )->m_flTurningInaccuracy();

						const auto max_speed_pair = memory::read<std::pair<float, float>>( weapon_vdata + SCHEMA_OFFSET( "CCSWeaponBaseVData", "m_flMaxSpeed"_hash ) );
						const auto inaccuracy_move_pair = memory::read<std::pair<float, float>>( weapon_vdata + SCHEMA_OFFSET( "CCSWeaponBaseVData", "m_flInaccuracyMove"_hash ) );
						const auto inaccuracy_jump_initial = reinterpret_cast<CCSWeaponBaseVData*>(weapon_vdata)->m_flInaccuracyJumpInitial();
						const auto inaccuracy_jump_apex = reinterpret_cast<CCSWeaponBaseVData*>(weapon_vdata)->m_flInaccuracyJumpApex();
						...
						const auto is_walking = reinterpret_cast<C_CSPlayerPawn*>( local_pawn )->m_bIsWalking();
		...
		const auto line_length = std::max( cfg.line_length.value, 24.0f );
		const auto thickness = std::max( cfg.thickness.value, 1.0f );
		const auto gap = std::max( cfg.gap.value, this->m_spread_smooth ) + ( 40.0f * ( 1.0f - this->m_scope_anim ) );
		const auto length = line_length * this->m_scope_anim;
		const auto inside_alpha = static_cast< std::uint8_t >( cfg.color.value.a * this->m_scope_anim );
		const auto outside_alpha = cfg.fade_in
			? static_cast< std::uint8_t >( 0 )
			: static_cast< std::uint8_t >( cfg.outside_color.value.a * this->m_scope_anim );

		const auto col_inside = xdraw::color{ cfg.color.value.r, cfg.color.value.g, cfg.color.value.b, inside_alpha };
		const auto col_outside = xdraw::color{ cfg.outside_color.value.r, cfg.outside_color.value.g, cfg.outside_color.value.b, outside_alpha };
		const auto col_start = cfg.fade_in
			? xdraw::color{ cfg.color.value.r, cfg.color.value.g, cfg.color.value.b, 0 }
			: col_inside;

		if ( cfg.glow && inside_alpha > 0 )
		{
			auto& glow = xdraw::get_glow( );
			const auto glow_inside_a = static_cast< std::uint8_t >( static_cast< float >( inside_alpha ) * cfg.glow_strength );
			const auto glow_outside_a = static_cast< std::uint8_t >( static_cast< float >( outside_alpha ) * cfg.glow_strength );
			const auto glow_inside = xdraw::color{ cfg.color.value.r, cfg.color.value.g, cfg.color.value.b, glow_inside_a };
			const auto glow_outside = xdraw::color{ cfg.outside_color.value.r, cfg.outside_color.value.g, cfg.outside_color.value.b, glow_outside_a };
			const auto glow_start = cfg.fade_in
				? xdraw::color{ cfg.color.value.r, cfg.color.value.g, cfg.color.value.b, 0 }
				: glow_inside;
			...
			const auto glow_draw_line = [ & ]( float x1, float y1, float x2, float y2 )
				{
					...
					const auto glow_thickness = thickness + 0.5f;

					const float pts[ ]{ x1, y1, mx, my, x2, y2 };
					const xdraw::color cols[ ]{ glow_start, glow_inside, glow_outside };
		...
		const auto draw_line = [ & ]( float x1, float y1, float x2, float y2 )
			{
				...
				const float pts[ ]{ x1, y1, mx, my, x2, y2 };
				const xdraw::color cols[ ]{ col_start, col_inside, col_outside };

				draw_list.polyline_gradient( pts, cols, false, thickness );
			};
	...
	void hud::do_velocity( xdraw::draw_list& draw_list, float cx, float screen_h, std::uintptr_t local_pawn )
	{
		...
		const auto velocity = reinterpret_cast<C_BaseEntity*>(local_pawn)->m_vecAbsVelocity();
