...
#include <valve/classes/debug_overlay.h>
#include <imgui.h>
...
namespace features::misc {
	...
	namespace detail {
		...
		[[nodiscard]] std::string format_hit_chat_message( const std::string& name, int damage, const std::string& hitgroup, int health, const std::string& reason = {} )
		{
			...
			const auto body_part = hitgroup.empty( ) ? "body" : hitgroup;

			if ( !reason.empty( ) )
			{
				return chat_dim( "Hit " ) + chat_white( name ) + chat_dim( " in the " ) + chat_white( body_part ) + chat_dim( " for " ) + chat_white( damage_str ) + chat_dim( std::format( " damage, {} ({} remaining)", reason, health ) );
			}

			return chat_dim( "Hit " ) + chat_white( name ) + chat_dim( " in the " ) + chat_white( body_part ) + chat_dim( " for " ) + chat_white( damage_str ) + chat_dim( std::format( " damage ({} remaining)", health ) );
		}

		[[nodiscard]] std::string format_knife_chat_message( const std::string& name, int damage, int health )
		{
			return chat_dim( "Hit " ) + chat_white( name ) + chat_dim( " in the knife for " ) + chat_white( std::to_string( damage ) ) + chat_dim( std::format( " damage ({} remaining)", health ) );
		}

		[[nodiscard]] std::string format_taser_chat_message( const std::string& name )
		{
			return chat_dim( "Hit " ) + chat_white( name ) + chat_dim( " in the body for 100 damage (0 remaining)" );
		}
		...
		void chat_print( const char* label_text, std::uint8_t sr, std::uint8_t sg, std::uint8_t sb, std::uint8_t er, std::uint8_t eg, std::uint8_t eb, const char* msg )
		{
			...
			const auto hud_element = memory::call<std::uintptr_t>( PATTERN(PATTERN_FIND_HUD_ELEMENT), xs( "CCSGO_HudVoiceStatus" ) );
			if ( !hud_element )
			{
				return;
			}
			...
			memory::call<void>( PATTERN(PATTERN_SET_VOICE_DATA), voice, buf, 0xFFFFFFFF, flags );
		}

		void chat_print_brand( const char* msg )
		{
			chat_print( "[Kitty.cc]", k_periwinkle_start_r, k_periwinkle_start_g, k_periwinkle_start_b, k_periwinkle_end_r, k_periwinkle_end_g, k_periwinkle_end_b, msg );
		}
	...
	void impacts::on_report_hit( std::uintptr_t msg )
	{
		...
		if ( !cfg.hit_marker.value && !cfg.hit_sound.value )
		{
			return;
		}
	...
	void impacts::on_player_hurt( std::uintptr_t event )
	{
...

		{
			...
			for ( auto& shot : this->m_pending_shots )
			{
				if ( shot.resolved )
					continue;
				if ( shot.victim_pawn == data.victim_pawn )
				{
					shot.resolved = true;
					break;
				}
		...
		if ( cfg.hit_marker )
		{
			...
			const auto game_scene_node = ( data.victim_pawn && memory::is_game_ptr( data.victim_pawn ) ) ? reinterpret_cast<C_BaseEntity*>(data.victim_pawn)->m_pGameSceneNode() : 0;
			if ( game_scene_node && memory::is_game_ptr( game_scene_node ) )
			{
				const auto origin = reinterpret_cast<CGameSceneNode*>(game_scene_node)->m_vecAbsOrigin();
				const auto view_offset = reinterpret_cast<C_BaseModelEntity*>(data.victim_pawn)->m_vecViewOffset();
		...
		features::world::g_hitkill_particles.on_player_hurt( data.victim_pawn, is_kill );

		if ( false && cfg.local_impact_boxes.value )
		{
			const auto game_scene_node = reinterpret_cast<C_BaseEntity*>( data.victim_pawn )->m_pGameSceneNode( );
			if ( game_scene_node )
			{
				const auto origin = reinterpret_cast<CGameSceneNode*>( game_scene_node )->m_vecAbsOrigin( );
				const auto view_offset = reinterpret_cast<C_BaseModelEntity*>( data.victim_pawn )->m_vecViewOffset( );
				const auto above = origin + view_offset * 0.75f;

				const auto size = static_cast<float>( cfg.local_impact_boxes_size.value );
				const auto duration = static_cast<float>( cfg.local_impact_boxes_duration.value );
				const auto server_col = cfg.local_impact_boxes_server_color.value;

			if ( auto dbg = valve::get_debug_overlay( ) )
				{
					const math::vector3 mins{ -size, -size, -size };
					const math::vector3 maxs{ size, size, size };
					const math::vector3 rot{ 0.0f, 0.0f, 0.0f };
					dbg->add_box_overlay( above, mins, maxs, rot, server_col, false, duration );
				}
			}
		}
	}
	...
	void impacts::on_bullet_impact( std::uintptr_t event )
	{
		...
		const auto controller = memory::call<std::uintptr_t>(PATTERN(PATTERN_GAME_EVENT_GET_CONTROLLER), event, &userid_key );
		...
		const auto x = memory::call<float>(PATTERN(PATTERN_GAME_EVENT_GET_FLOAT), event, "x", 0.0f );
		const auto y = memory::call<float>(PATTERN(PATTERN_GAME_EVENT_GET_FLOAT), event, "y", 0.0f );
		const auto z = memory::call<float>(PATTERN(PATTERN_GAME_EVENT_GET_FLOAT), event, "z", 0.0f );
		...
		if ( cfg.local_impact_boxes.value )
		{
			const auto size = static_cast<float>( cfg.local_impact_boxes_size.value );
			const auto duration = static_cast<float>( cfg.local_impact_boxes_duration.value );
			const auto client_col = cfg.local_impact_boxes_client_color.value;
			const auto server_col = cfg.local_impact_boxes_server_color.value;
		...
		if ( auto dbg = valve::get_debug_overlay( ) )
			{
				...
				dbg->add_box_overlay( pos, mins, maxs, rot, client_col, false, duration );
				dbg->add_box_overlay( pos, mins, maxs, rot, server_col, true, duration );
			}
		}
	}
	...
	{
		const auto owner_handle = reinterpret_cast<C_BaseEntity*>(weapon)->m_hOwnerEntity();
	...
	void impacts::on_get_interpolated_shoot_position( std::uintptr_t weapon_services, float* out )
	{
		...
		const auto local_weapon_services = reinterpret_cast<C_BasePlayerPawn*>(local_pawn)->m_pWeaponServices();
	...
	void impacts::on_boom( std::uintptr_t victim_pawn, int hitgroup, float damage, float hitchance, float inaccuracy, float spread, const math::vector3& aim_angle, const math::vector3& shoot_position, int tick, const std::array<systems::bones::data, 27>& skeleton, bool forced )
	{
		...
		const auto target_velocity = reinterpret_cast<C_BaseEntity*>(victim_pawn)->m_vecVelocity();
	...
	impacts::hit_data impacts::parse_event( std::uintptr_t event )
	{
		...
		const auto damage = memory::call<int>( PATTERN(PATTERN_GAME_EVENT_GET_INT), event, "dmg_health", false );
		const auto hitgroup = memory::call<int>( PATTERN(PATTERN_GAME_EVENT_GET_INT), event, "hitgroup", false );
		...
		if ( !was_aimbot )
		{
			...
			if ( weapon_services )
			{
				...
				if ( weapon )
				{
					...
					if ( weapon_vdata )
					{
						weapon_type = reinterpret_cast<CCSWeaponBaseVData*>(weapon_vdata)->m_WeaponType();
						}
					}
				}
			}
		...
		return
		{
			...
			.victim_pawn = victim_pawn,
			.damage = damage,
			.health = memory::call<int>( PATTERN(PATTERN_GAME_EVENT_GET_INT), event, "health", false ),
			.hitgroup = hitgroup,
	...
	{
		const auto name_ptr = reinterpret_cast<CCSPlayerController*>(controller)->m_sSanitizedPlayerName();
		if ( !name_ptr )
		{
			return "unknown";
		}
	...
	{
		const auto controller_handle = reinterpret_cast<C_BasePlayerPawn*>(pawn)->m_hController();
	...
	float impacts::distance_to_nearest_hitbox( const shot_record& shot ) const
	{
		...
		const auto game_scene_node = reinterpret_cast<C_BaseEntity*>(shot.victim_pawn)->m_pGameSceneNode();
		if ( !game_scene_node )
		{
			return -1.0f;
		}
	...
	void impacts::add_hit_log( const hit_data& data )
	{
		...
		if ( cfg.chat_log.value )
		{
			...
			if ( cfg.chat_log.value )
			{
				detail::chat_print_brand( chat_msg.c_str( ) );
			}
		}
	...
	if ( reason && std::string_view( reason ) == "lag compensation" )
		{
			const auto hitbox_dist = this->distance_to_nearest_hitbox( shot );
			const auto vel_len = shot.target_velocity.length( );
			const auto delta_t = shot.impact_time - shot.time;
	...
	void impacts::render_hit_markers( xdraw::draw_list& draw_list, float time )
	{
		...
		{
			const auto duration = ( cfg.hit_marker_duration.value > 0.1f ) ? cfg.hit_marker_duration.value : 1.2f;
			...
			math::vector3 elevated_pos = it->position;
			elevated_pos.z += elapsed * 24.0f;

			const auto screen = systems::g_view.project( elevated_pos );
			const auto screen_fly_up = elapsed * 28.0f;

			const auto x = screen.x, y = screen.y - screen_fly_up;
			...
			if ( show_damage )
			{
				const auto base_offset = show_classic ? 16.0f : 0.0f;
			...
			if ( cfg.hit_marker_glow && alpha > 0 )
			{
				...
				if ( show_classic )
				{
					for ( auto t = 0; t < 3; ++t )
					{
						...
						draw_arm( glow, screen.x - size, screen.y - size, screen.x - gap, screen.y - gap, glow_col, glow_thick );
						draw_arm( glow, screen.x + size, screen.y - size, screen.x + gap, screen.y - gap, glow_col, glow_thick );
						draw_arm( glow, screen.x - size, screen.y + size, screen.x - gap, screen.y + gap, glow_col, glow_thick );
						draw_arm( glow, screen.x + size, screen.y + size, screen.x + gap, screen.y + gap, glow_col, glow_thick );
					}
			...
			if ( show_classic )
			{
				draw_arm( draw_list, screen.x - size, screen.y - size, screen.x - gap, screen.y - gap, color, thickness );
				draw_arm( draw_list, screen.x + size, screen.y - size, screen.x + gap, screen.y - gap, color, thickness );
				draw_arm( draw_list, screen.x - size, screen.y + size, screen.x - gap, screen.y + gap, color, thickness );
				draw_arm( draw_list, screen.x + size, screen.y + size, screen.x + gap, screen.y + gap, color, thickness );
			}
			...
			if ( show_damage )
			{
				const auto shadow_col = xdraw::color{ 0, 0, 0, alpha };
				draw_list.text( draw_x + 1.0f, draw_y + 1.0f, damage_text, shadow_col );
				draw_list.text( draw_x - 1.0f, draw_y - 1.0f, damage_text, shadow_col );
	...
	void impacts::render_logs( xdraw::draw_list& draw_list, float time )
	{
		...
		const bool have_ui = fg && text_font && icon_font && text_font->IsLoaded( );

		const auto miss_accent = xdraw::color{ 255, 150, 150, 255 };
		...
		visible.reserve( this->m_logs.size( ) );
		...
		for ( auto it = this->m_logs.begin( ); it != this->m_logs.end( ); )
		{
			...
			{
				visible_entry ve{};
				ve.is_miss = it->is_miss;
				ve.alpha = alpha;

				const auto hitgroup = it->hitgroup;
				if ( it->is_miss )
				{
					ve.text = "missed " + it->name;
					if ( !hitgroup.empty( ) && hitgroup.find( '%' ) == std::string::npos )
					{
						ve.text += " (" + hitgroup + ")";
					}
					if ( !it->reason.empty( ) )
				...
				{
					const auto body_part = hitgroup.empty( ) ? "body" : hitgroup;
					ve.text = std::format( "Hit {} in the {} for {} damage ({} remaining)", it->name, body_part, it->damage, it->health );
				}

				visible.push_back( std::move( ve ) );
			}

			++it;
		}

		if ( !have_ui || visible.empty( ) )
		{
			return;
		}

		const float row_h = SCALE( 16.0f );
		const float pos_x = SCALE( 10.0f );
		float row_y = SCALE( 10.0f );

		for ( const auto& ve : visible )
		{
			const int a = static_cast<int>( ve.alpha * 255.0f );
			fg->AddText( text_font, 13.0f, ImVec2( pos_x, row_y ), IM_COL32( 255, 255, 255, a ), ve.text.c_str( ) );
			row_y += row_h;
		}
	}
	...
	namespace custom_sound_detail {
		...
		[[nodiscard]] std::wstring sounds_directory( )
		{
			...
			const auto root = std::wstring( app_data ) + L"\\cs2_internal";
...
void play_engine_path( const char* sound_path, float volume )
		{
			...
			memory::call<void>( PATTERN(PATTERN_PLAY_SOUND), 0.0f, &args );
		}
	...
	std::string impacts::custom_sounds_directory_narrow( )
	{
		...
		return custom_sound_detail::wide_to_utf8( directory );
	}
	...
	void impacts::play_bullet_impact_effect( const math::vector3& position )
	{
		...
		memory::call<bool>(PATTERN(PATTERN_PARTICLE_SET_CONTROL_POINT), particle_manager, effect_index, 0, &position, 0 );
		...
		memory::call<bool>(PATTERN(PATTERN_PARTICLE_SET_CONTROL_POINT), particle_manager, effect_index, 1, &color, 0 );
	}
	...
	void impacts::play_bullet_tracer( const math::vector3& position )
	{
		...
		memory::call<bool>( PATTERN(PATTERN_PARTICLE_SET_CONTROL_POINT), particle_manager, effect_index, 0, &this->m_buffered_eye_position, 0 );
		memory::call<bool>( PATTERN(PATTERN_PARTICLE_SET_CONTROL_POINT), particle_manager, effect_index, 1, &position, 0 );
		...
		memory::call<bool>( PATTERN(PATTERN_PARTICLE_SET_CONTROL_POINT), particle_manager, effect_index, 2, &color, 0 );

		const math::vector3 lifetime{ cfg.bullet_tracer_duration, 0.0f, 0.0f };
		memory::call<bool>( PATTERN(PATTERN_PARTICLE_SET_CONTROL_POINT), particle_manager, effect_index, 3, &lifetime, 0 );
	}
