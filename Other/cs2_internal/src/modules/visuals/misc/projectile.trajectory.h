...
namespace features::misc {
	...
	void projectile_trajectory::on_render( xdraw::draw_list& draw_list )
	{
		...
		const auto local_pawn_handle = systems::g_entities.player_pawn_handle( local.controller );
		...
		for ( auto& gren : this->m_in_flight )
		{
			...
			if ( alpha > 0.0f )
			{
				this->render_trajectory( draw_list, gren.traj, alpha, true, local, gren.weapon_hash );
			}
		}
	...
	void projectile_trajectory::on_create_move( systems::input::usercmd* cmd )
	{
		...
		const auto pin_pulled = reinterpret_cast<C_BaseCSGrenade*>(ctx.weapon)->m_bPinPulled();
		const auto throw_time = reinterpret_cast<C_BaseCSGrenade*>(ctx.weapon)->m_fThrowTime();
		...
		else if ( pin_pulled && !holding_attack && settings::g_misc.m_projectile_trajectory.straight_throw.value )
		{
			if ( !this->m_delay_release )
			{
				...
				this->m_delay_ticks = 0;
				this->m_delayed_strength = std::clamp( reinterpret_cast<C_BaseCSGrenade*>(ctx.weapon)->m_flThrowStrength(), 0.0f, 1.0f );
				this->m_delayed_attack2 = !attacking && attacking2;
			}

			auto predicted_velocity = systems::g_prediction.pre( ).networked_velocity;
			systems::g_prediction.simulate( cmd, local, [ & ]( ) { predicted_velocity = reinterpret_cast<C_BaseEntity*>(local.pawn)->m_vecVelocity(); } );
	...
	void projectile_trajectory::setup_throw( std::uintptr_t local_pawn, std::uintptr_t weapon, math::vector3& origin, math::vector3& velocity )
	{
		...
		const auto pin_pulled = reinterpret_cast<C_BaseCSGrenade*>(weapon)->m_bPinPulled();
		const auto throw_time = reinterpret_cast<C_BaseCSGrenade*>(weapon)->m_fThrowTime();

		if ( pin_pulled || throw_time > 0.0f )
		{
			strength = std::clamp( reinterpret_cast<C_BaseCSGrenade*>(weapon)->m_flThrowStrength(), 0.0f, 1.0f );
		...
		auto eye_pos = systems::g_frame_data.origin( ) + reinterpret_cast<C_BaseModelEntity*>(local_pawn)->m_vecViewOffset();
	...
	void projectile_trajectory::resolve_collision( const systems::tracing::result& trace, math::vector3& pos, math::vector3& vel, std::uintptr_t thrower_pawn, int sim_tick, bool& detonated ) const
	{
		...
		if ( trace.hit_entity )
		{
			if ( !grenade_passes_through_hit_entity( trace.hit_entity ) )
			{
				const auto health = reinterpret_cast<C_BaseEntity*>(trace.hit_entity)->m_iHealth();
	...
	void projectile_trajectory::update_weapon_properties( std::uintptr_t weapon, std::uintptr_t weapon_vdata )
	{
		...
		this->m_throw_velocity = std::clamp( reinterpret_cast<CCSWeaponBaseVData*>(weapon_vdata)->m_flThrowVelocity(), 1.0f, 10000.0f );

		const auto name_ptr = reinterpret_cast<CCSWeaponBaseVData*>(weapon_vdata)->m_szName();
		if ( !name_ptr )
		{
	...
	void projectile_trajectory::update_in_flight( const systems::local::snapshot& local )
	{
		...
		for ( const auto& entry : projectiles )
		{
			...
			const auto thrower_handle = reinterpret_cast<C_BaseGrenade*>(entry.ptr)->m_hThrower();
			auto is_enemy{ true };
			...
			else if ( thrower_handle && thrower_handle != 0xffffffff )
			{
				...
				if ( thrower )
				{
					const auto thrower_team = reinterpret_cast<C_BaseEntity*>(thrower)->m_iTeamNum();
			...
			if ( existing )
			{
				...
				{
					const auto initial_pos = reinterpret_cast<C_BaseCSGrenadeProjectile*>(entry.ptr)->m_vInitialPosition();
					const auto initial_vel = reinterpret_cast<C_BaseCSGrenadeProjectile*>(entry.ptr)->m_vInitialVelocity();
				...
				if ( !existing->detonated )
				{
					...
					case "weapon_hegrenade"_hash:
					case "weapon_flashbang"_hash:
						effect_started = reinterpret_cast<C_BaseCSGrenadeProjectile*>(entry.ptr)->m_nExplodeEffectTickBegin() > 0;
						break;
					case "weapon_smokegrenade"_hash:
						effect_started = reinterpret_cast<C_SmokeGrenadeProjectile*>(entry.ptr)->m_nSmokeEffectTickBegin() > 0;
						break;
					case "weapon_decoy"_hash:
						effect_started = reinterpret_cast<C_DecoyProjectile*>(entry.ptr)->m_nDecoyShotTick() > 0;
						break;
					default:
						break;
					}
...

			{
				...
				case "weapon_hegrenade"_hash:
				case "weapon_flashbang"_hash:
					already_detonated = reinterpret_cast<C_BaseCSGrenadeProjectile*>(entry.ptr)->m_nExplodeEffectTickBegin() > 0;
					break;
				case "weapon_smokegrenade"_hash:
					already_detonated = reinterpret_cast<C_SmokeGrenadeProjectile*>(entry.ptr)->m_nSmokeEffectTickBegin() > 0;
					break;
				case "weapon_decoy"_hash:
					already_detonated = reinterpret_cast<C_DecoyProjectile*>(entry.ptr)->m_nDecoyShotTick() > 0;
					break;
				default:
					break;
				}
	...
	void projectile_trajectory::correct_throw_angles( systems::input::usercmd* cmd, const systems::local::snapshot& local, std::uintptr_t weapon )
	{
		...
		auto strength = std::clamp( reinterpret_cast<C_BaseCSGrenade*>(weapon)->m_flThrowStrength(), 0.0f, 1.0f );
		if ( std::abs( strength - 0.5f ) <= 0.1f )
		{
			strength = 0.5f;
		}
	...
	void projectile_trajectory::compute_damage_at_endpoint( const trajectory& traj, std::vector<damage_info>& damages, const systems::local::snapshot& local ) const
	{
		...
		for ( const auto& player : players )
		{
			...
			if ( !reinterpret_cast<CCSPlayerController*>(player.ptr)->m_bPawnIsAlive() )
			{
				continue;
			}
			...
			const auto player_origin = reinterpret_cast<CGameSceneNode*>(game_scene_node)->m_vecAbsOrigin();
			...
			const auto armor = reinterpret_cast<C_CSPlayerPawn*>( pawn )->m_ArmorValue();
	...
	void projectile_trajectory::render_preview( xdraw::draw_list& draw_list, const systems::local::snapshot& local )
	{
		...
		if ( frame_traj.valid )
		{
			this->render_trajectory( draw_list, frame_traj, 1.0f, false, local, this->m_weapon_hash );
		}
	}

	void projectile_trajectory::render_trajectory( xdraw::draw_list& draw_list, const trajectory& traj, float alpha, bool is_thrown, const systems::local::snapshot& local, std::uintptr_t weapon_hash ) const
	{
		...
		const auto& held = ( cfg.held_color.value.a > 0 ) ? cfg.held_color : cfg.color;
		const auto color = is_thrown
			? ( has_damage ? cfg.will_deal_damage_thrown_color : cfg.thrown_color )
			: ( has_damage ? cfg.will_deal_damage_held_color : held );
		...
		const auto glow_enabled = cfg.glow.value || cfg.effect.value;
		if ( glow_enabled && alpha > 0.0f )
		{
			...
			const auto strength = std::clamp( cfg.glow_strength.value > 0.0f ? cfg.glow_strength.value : 1.0f, 0.0f, 1.0f );
		...
		if ( traj.end_tick >= 0 )
		{
			...
			if ( systems::g_view.projection_valid( screen ) )
			{
				...
				if ( weapon_hash == "weapon_hegrenade"_hash )
					icon_name = "hegrenade";
				else if ( weapon_hash == "weapon_flashbang"_hash )
					icon_name = "flashbang";
				else if ( weapon_hash == "weapon_smokegrenade"_hash )
					icon_name = "smokegrenade";
				else if ( weapon_hash == "weapon_molotov"_hash )
					icon_name = "molotov";
				else if ( weapon_hash == "weapon_incgrenade"_hash )
					icon_name = "incgrenade";
				else if ( weapon_hash == "weapon_decoy"_hash )
					icon_name = "decoy";

				if ( icon_name )
				{
					const auto ico = systems::g_icons.get( icon_name, 0.45f );
					if ( ico && ico->texture )
					{
						...
						const auto iy = std::floorf( screen.y - ih - 6.0f );
						const auto icon_a = static_cast< std::uint8_t >( 255.0f * alpha );
						const auto outline = xdraw::color{ 0, 0, 0, icon_a };
						const auto tint = xdraw::color{ color.value.r, color.value.g, color.value.b, icon_a };

						draw_list.image( ix - 1.0f, iy, iw, ih, ico->texture.Get( ), outline );
						draw_list.image( ix + 1.0f, iy, iw, ih, ico->texture.Get( ), outline );
						draw_list.image( ix, iy - 1.0f, iw, ih, ico->texture.Get( ), outline );
						draw_list.image( ix, iy + 1.0f, iw, ih, ico->texture.Get( ), outline );
						draw_list.image( ix, iy, iw, ih, ico->texture.Get( ), tint );
					}
				}
			}

			for ( const auto& dmg : damages )
			{
				const auto dmg_screen = systems::g_view.project( dmg.position );
				if ( !systems::g_view.projection_valid( dmg_screen ) )
				{
					continue;
				}
				...
				{
					std::snprintf( text, sizeof( text ), "%d", dmg.damage );
				}

				const auto col = dmg.is_lethal
					? xdraw::color{ 255, 0, 0, static_cast< std::uint8_t >( 255 * alpha ) }
					: xdraw::color{ 255, 255, 50, static_cast< std::uint8_t >( 255 * alpha ) };

				const auto [tw, th] = xdraw::measure_text( text );
				(void)th;
				draw_list.text( dmg_screen.x - tw * 0.5f, dmg_screen.y + 12.0f, text, col, xdraw::text_style::outlined );
			}
		}
	}
