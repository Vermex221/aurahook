...
namespace features::esp {
	...
	namespace player {
		...
		public:
			...
			public:
				...
				[[nodiscard]] std::uint32_t get_generation( std::uintptr_t pawn ) const;
				[[nodiscard]] bool matches( std::uintptr_t pawn, std::uintptr_t scene_object, std::uint32_t generation ) const;
				...
				struct object {
					...
					[[nodiscard]] bool setup_bones( const systems::bones::data* bones, int count ) const;
					[[nodiscard]] bool usable( ) const;
				};

			private:
				void defer_destroy( object& obj );
				void flush_deferred( );

				mutable std::mutex m_mutex{};
				std::unordered_map<std::uintptr_t, object> m_objects{};
				std::vector<std::uintptr_t> m_deferred_destroy{};
				std::uint32_t m_generation_clock{ 1 };
				int m_destroy_delay{};
			};
			...
			private:
				...
				struct pose {
					std::array<systems::bones::data, k_max_bones> bones{};
					int bone_count{};
				};

				struct entry {
					backtrack::object visual{};
					pose shot_pose{};
					bool pose_applied{};
				};

				void defer_destroy( backtrack::object& obj );
				void flush_deferred( );

				mutable std::mutex m_mutex{};
				std::unordered_map<std::uintptr_t, pose> m_pending{};
		...
		private:
			void apply_layer( std::uintptr_t primitive_buffer, void( __fastcall* original_fn )( std::uintptr_t, std::uintptr_t, std::uintptr_t, std::uintptr_t ), std::uintptr_t a1, std::uintptr_t scene_object, std::uintptr_t scene_view, const xdraw::color& color, settings::esp::cham_ids material_id, bool occluded = false );
			...
			void apply_onshot( std::uintptr_t pawn, std::uintptr_t os_obj, std::uintptr_t primitive_buffer, void( __fastcall* original_fn )( std::uintptr_t, std::uintptr_t, std::uintptr_t, std::uintptr_t ), std::uintptr_t a1, std::uintptr_t scene_view );
		...
		public:
			struct preview_input
			{
				...
				bool has_defuser{ true };
				bool is_scoped{ true };
				bool is_defusing{ true };
				bool is_flashed{ true };
				float name_offset{ 0.0f };
				std::string name{ "Player" };
				std::string weapon_name{};
				std::array<math::vector2, 27> bone_screen{};
				std::array<bool, 27> bone_valid{};
				bool has_bones{ false };
			};

			void on_render( xdraw::draw_list& draw_list );
			void on_render_preview( xdraw::draw_list& draw_list, const settings::esp::player::overlay& cfg, const preview_input& input );
		...
		private:
			...
			void add_skeleton_screen( xdraw::draw_list& draw_list, const preview_input& input, const settings::esp::player::overlay::skeleton& cfg, bool visible );
			...
			void add_name( xdraw::draw_list& draw_list, const systems::bounds::data& bounds, const info& info, const settings::esp::player::overlay::name& cfg, draw_offsets& offsets, float extra_offset = 0.0f );
			...
			[[nodiscard]] bool cached_visibility( std::uintptr_t pawn ) const;
			...
			std::array<std::unordered_map<std::uintptr_t, bool>, 2> m_visibility{};
			std::atomic<int> m_visibility_read{ 0 };
		};
	...
	namespace item {
		...
		private:
			...
			void add_label( xdraw::draw_list& draw_list, const info& info, const settings::esp::item::overlay::group& cfg, float fade_alpha );

			[[nodiscard]] info get_info( const systems::entities::cached& entity );
			[[nodiscard]] std::uint32_t get_item_group( std::uint32_t schema_hash );

			std::unordered_map<std::uintptr_t, float> m_fade_alpha{};
	...
	namespace projectile {
		...
		private:
			...
			void add_proximity_warning( xdraw::draw_list& draw_list, const systems::entities::cached& entity, const info& info );
	...
	namespace other {
		...
		private:
			void add_bomb( xdraw::draw_list& draw_list, float appear_alpha );
			void add_spectators( xdraw::draw_list& draw_list );
			void add_chickens( xdraw::draw_list& draw_list );

			std::unordered_map<std::uintptr_t, float> m_fade_alpha{};
			std::chrono::steady_clock::time_point m_last_tick{};
			float m_bomb_appear_alpha{};
		};
