...
namespace features::esp::item {
	...
	bool chams::on_generate_primitives( std::uintptr_t, std::uint32_t, std::uintptr_t, std::uintptr_t, void( __fastcall* )( std::uintptr_t, std::uintptr_t, std::uintptr_t, std::uintptr_t ), std::uintptr_t, std::uintptr_t )
	{
		return false;
	}
	...
	{
		if ( !detail::scene_object_live( scene_object ) )
		{
			return;
		}
