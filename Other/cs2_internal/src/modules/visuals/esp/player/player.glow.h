...
namespace features::esp::player {
	...
	bool glow::on_is_glowing( std::uintptr_t owner_entity, std::uint32_t owner_hash ) const
	{
		...
		const auto team = reinterpret_cast<C_BaseEntity*>( owner_entity )->m_iTeamNum( );
		const auto health = reinterpret_cast<C_BaseEntity*>( owner_entity )->m_iHealth( );

		const auto is_other_team = !local.is_team_mode || team != local.team;
		const auto is_local = owner_entity == local.pawn;
