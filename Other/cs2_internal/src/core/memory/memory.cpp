...
namespace {
	...
	__forceinline void* game_alloc( std::size_t size )
	{
	...
	__forceinline void game_free( void* ptr )
	{
		const auto memalloc = *reinterpret_cast<void**>( MODULE_EXPORT( "tier0.dll:g_pMemAlloc" ) );
		const auto vtable = *reinterpret_cast<void***>( memalloc );
...
namespace memory {
	...
	std::uintptr_t resolve_pattern (std::string_view pattern) {
		...
		const auto first_byte_vec = _mm_set1_epi8 (static_cast<char>(first_byte));

		const auto module_end = module_base + module_size;
		const auto scan_end = (module_size >= 16) ? module_size - 16 : 0;

		auto match {0ull};

		for (auto i = 0ull; i <= scan_end; i += 16) {
			const auto memory_chunk = _mm_loadu_si128 (reinterpret_cast<const __m128i*>(module_base + i));
			const auto comparison = _mm_cmpeq_epi8 (memory_chunk, first_byte_vec);
			auto mask = _mm_movemask_epi8 (comparison);
...
found_type_descriptor:
	...
	std::uintptr_t get_vfunc (std::uintptr_t instance, std::size_t index) {
		if (!instance || !is_game_ptr(instance)) {
			return 0;
		}

		const auto vtable = safe_read<std::uintptr_t>(instance);
		if (!vtable || !is_game_ptr(*vtable)) {
			return 0;
		}

		const auto func = safe_read<std::uintptr_t>(*vtable + index * sizeof (std::uintptr_t));
		if (!func || !detail::is_user_addr(*func)) {
			return 0;
		}
